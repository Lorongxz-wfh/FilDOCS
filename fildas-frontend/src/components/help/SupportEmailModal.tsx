      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
          <div className="space-y-1.5">
            <label className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest pl-1">
              Subject
            </label>
            <input
              type="text"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between pl-1">
              <label className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
              ...
              {attachments.length < 5 && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-semibold text-brand-600 dark:text-brand-400 hover:text-brand-700 transition-colors"
                >
            ...
            {previews.length > 0 ? (
              <div className="grid grid-cols-5 gap-3">
                {previews.map((url, i) => (
                  <div
                    key={url}
                    className="group relative aspect-square rounded-xl border border-slate-200 dark:border-surface-400 bg-slate-50 dark:bg-surface-600 overflow-hidden "
                  >
                    <img
                      src={url}
                      alt="preview"
              ...
              >
                <div className="p-3 rounded-full bg-white dark:bg-surface-400  border border-slate-100 dark:border-surface-300 mb-3 group-hover:scale-110 transition-transform">
        ...
        <div className="border-t border-slate-100 dark:border-surface-400 px-6 py-4 bg-slate-50/50 dark:bg-surface-600/50 flex items-center justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 text-sm font-semibold text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-surface-400 rounded-xl transition-all"
          >
            Cancel
          </button>
          <Button
            type="submit"
            variant="primary"
            className="px-8 py-2.5 rounded-xl shadow-lg shadow-brand-500/20 font-semibold"
            loading={sending}
            onClick={(e) => {
