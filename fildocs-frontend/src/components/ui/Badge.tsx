...
export function StatusBadge({
  ...
  return (
    <span
      className={`inline-flex items-center rounded px-1.5 py-0.5 text-[11px] font-semibold leading-tight transition-colors ${cls} ${className}`}
    >
      {status}
    </span>
  );
...
export function TypePill({
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full bg-neutral-100 px-2.5 py-0.5 text-[11px] font-semibold text-neutral-600 dark:bg-surface-400 dark:text-neutral-400 ${className}`}>
      {icon}
      {label}
    </span>
  );
...
export function AnnouncementTypePill({ type, className = "" }: { type: string, className?: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-md px-1.5 py-0.5 text-[11px] font-semibold ${cls} ${className}`}
    >
      {label}
    </span>
  );
