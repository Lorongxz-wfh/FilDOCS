...
const UsersTab: React.FC<UsersTabProps> = ({ adminUserLoading, adminUserStats }) => {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-md border border-slate-200 dark:border-surface-400 bg-white dark:bg-surface-500 p-6 ">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50 mb-6 flex items-center gap-2">
              <ShieldCheck size={16} className="text-brand-500" /> Account Verification
            </h3>
          ...
          <div className="rounded-md border border-dashed border-slate-200 dark:border-surface-400 bg-slate-50/50 dark:bg-surface-600/20 p-8 flex flex-col items-center justify-center text-center">
            <Mail className="h-8 w-8 text-slate-300 mb-3" />
            <p className="text-xs font-medium text-slate-400 dark:text-slate-500 max-w-sm">
              Role-based population trends and average user session time metrics are being calculated in the background
              and will appear here in the next update.
            </p>
          </div>
        </div>
      </div>
