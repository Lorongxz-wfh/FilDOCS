...
const ExecutiveTab: React.FC<ExecutiveTabProps> = ({ loading, stats }) => {
  ...
  const yieldRating =
    kpis.first_pass_yield_pct > 90
      ? "Excellence"
      : kpis.first_pass_yield_pct > 75
      ? "Target"
      : "Monitoring";
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
            <ShieldCheck className="h-5 w-5 text-brand-500" /> Institutional Executive Summary
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            High-level institutional health and efficiency metrics
          </p>
        </div>
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-4 py-2">
          <span className="text-[10px] uppercase font-semibold text-emerald-600 dark:text-emerald-400 block tracking-wider">
            Quality Score
          </span>
